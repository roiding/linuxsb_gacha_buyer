      ...
      <section id="tab-settings" class="tab">
        ...
        <form id="cfg-form" class="settings-grid">
          ...
          <fieldset><legend>余额与节奏</legend><p class="hint">不限制总采购额度和每轮数量，只保留账号余额保护线。</p>
            <div class="field-grid"><label>余额保护线<input name="min_balance" type="number" min="0" class="num"></label><label>扫描间隔（秒）<input name="scan_sec" type="number" min="1" class="num"></label><label>扫描方式<select name="scan_mode">
              <option value="">自动（间隔 ≤10s 只扫默认页，否则分类价格升序）</option>
              <option value="fast">只扫默认页（快，每轮 1 个请求）</option>
              <option value="thorough">分类价格升序（彻底，覆盖限价内便宜存量）</option>
            </select><small>自动档按扫描间隔切换：≤10 秒用快扫（快），>10 秒用分类扫描（彻底）</small></label></div>
          </fieldset>
